...
def test_calculate_posterior_unnormalized_probabilities_are_probabilities(default_classifier):
    res_true = default_classifier.calculate_posterior("1", "yes", normalize=False)
    assert (0 <= res_true['posterior']).all() and (res_true['posterior'] <= 1).all()
    res_false = default_classifier.calculate_posterior("1", "no", normalize=False)
    assert (0 <= res_false['posterior']).all() and (res_false['posterior'] <= 1).all()
    res_false = default_classifier.calculate_posterior("1", "skip", normalize=False)
    assert (0 <= res_false['posterior']).all() and (res_false['posterior'] == 1).all()
...
def test_skip_returns_prior_in_calculate_posterior(default_classifier):
    ...
    n_attributes = len(default_classifier.observations.columns)-1
